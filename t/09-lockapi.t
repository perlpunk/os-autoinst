...
[qr/mutex create 'lock1'/], 'mutex create lock STDERR ok';
ok(check_action('POST', 'mutex', {name => 'lock1'}), 'mutex_create request valid');
...
[qr/mutex unlock 'lock1'/], 'mutex unlock STDERR ok';
ok(check_action('POST', 'mutex/lock1', {action => 'unlock'}), 'mutex_unlock request valid');
...
[qr/barrier create 'barrier1' for 3 tasks/], 'barrier create STDERR ok';
ok(check_action('POST', 'barrier', {name => 'barrier1', tasks => 3}), 'barrier create request valid');
my $qr = [qr/barrier wait 'barrier1'/, qr/\Qtestapi::record_info(output="Wait for barrier1 (on parent job)", result="ok", title="Paused")\E/, qr/\Qtestapi::record_info(output="Wait for barrier1 (on parent job)", result="ok", title="Paused 0m0s")\E/];
...
[qr/barrier destroy 'barrier1'/], 'barrier destroy STDERR ok';
...
[qr/barrier create 'barrier1' for 3 tasks/],
...
[qr/barrier try wait 'barrier1'/], 'barrier try wait STDERR ok';
ok(check_action('POST', 'barrier/barrier1', undef), 'barrier wait request valid');
